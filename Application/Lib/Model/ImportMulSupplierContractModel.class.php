 ...
 * 
 */
class ImportMulSupplierContractModel extends BaseImportExcelModel
{
    protected $trueTableName = 'tb_crm_contract';
    public static $allSupplierContract;
    ...
    protected $_auto = [
        ...
        ['CRM_CON_TYPE', 0, Model::MODEL_INSERT],
    ];
    ...
    public function valid($row_index, $column_index, $value)
    {
        ...
        if ($db_field == 'CON_NO' and !empty($value)) {        
            if (in_array($value, $this->getAllSupplierContract())) $this->errorinfo [][$row_index.$column_index] = $this->title [$column_index]['en_name'] . ': ' . $value . ' 已存在';
     ...
     * 
     */
    public function getAllSupplierContract()
    {
        if (self::$allSupplierContract) return self::$allSupplierContract;
        $ret = $this->field('CON_NO')->where('CRM_CON_TYPE = 0')->select();
        if ($ret) self::$allSupplierContract = array_column($ret, 'CON_NO');
        return self::$allSupplierContract;
    }
