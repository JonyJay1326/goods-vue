/**
用法
<script type="text/javascript">$.fn.ce_area(['s_a','s_b','s_c'],['省', '市', '县'],["上海市","上海市","徐汇区"]);</script>
**/

(function ($) {
    $.fn.ce_area = function(ssss,wwww,kkkk){
        var s = ssss;
        var w = wwww;
		var k = kkkk;
        var _Dsy = function(){
        ...
        _Dsy.prototype.add = function(id,iArray){
        	this.Items[id] = iArray;
        };
        _Dsy.prototype.Exists = function(id){
...
dsy.add("0",["北京市","天津市","上海市","重庆市","河北省","山西省","内蒙古","辽宁省","吉林省","黑龙江省","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西","海南省","四川省","贵州省","云南省","西藏","陕西省","甘肃省","青海省","宁夏","新疆","香港","澳门","台湾省"]);
        ...
        // w 默认值
        // k 初始化值
        this.set_me = function setOptions(v,s,w){
        	var str="0";
        	for(i=0;i<v;i++){
        	...
        	with(ss){
        		length = 0;
        		options[0]=new Option(w[v],"");
        		if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v){
        			if(dsy.Exists(str)){
        				ar = dsy.Items[str];
        				for(i=0;i<ar.length;i++){
        					options[length]=new Option(ar[i],ar[i]);
							if(k[v]!=null&&ar[i]==k[v]){
								ss.options[i+1].selected = true;
							}
        				}//end for
        				//if(v){ options[0].selected = true; }
        			}
        		}//end if v
        		if(++v<s.length){setOptions(v,s,w);}
        	}//End with
        };
        ...
        {
        	this.set_me(0,s,w);
        	$(document.getElementById(s[0])).bind('change',function(){
                xme.set_me(1,s,w);
            });
            $(document.getElementById(s[1])).bind('change',function(){
                xme.set_me(2,s,w);
            });
        };
    };
